...
import type { FhirBundleContract, FhirExportContextContract } from '@shared/api/contracts';
...
export default function PatientDetails() {
  ...
  const handleExportFHIR = async () => {
    ...
    const scopedVisit = doctorVisits.find(v => v.patient_id === activePatientId);
    if (!scopedVisit?.hospital_id) {
      toast.error('FHIR export requires an active hospital context for this patient.');
      return;
    }
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const params: FhirExportContextContract = {
        purpose: 'TREATMENT',
        hospital_id: scopedVisit.hospital_id,
      };
      const res = await api.get<FhirBundleContract>(
        `/api/interoperability/patients/${activePatientId}/export`,
        { headers, params },
      );
      ...
      console.error(error);
      toast.error('FHIR export denied or failed. Confirm the patient has active scoped consent for this hospital and treatment purpose.');
    } finally {
      setExporting(false);
    }
  };
